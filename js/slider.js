...
const darkenEffect = document.getElementById("darkenEffect");

document.addEventListener("mousemove", (e) => {
  const x = e.clientX;
  const y = e.clientY;
  const maxDistance = 150; // максимальное расстояние, на котором градиент будет темнеть

  // Вычисляем дистанцию от курсора до границ страницы
  const offsetX = Math.min(x, window.innerWidth - x);
  const offsetY = Math.min(y, window.innerHeight - y);

  // Вычисляем степень затемнения
  const opacityX = Math.max(0, 1 - offsetX / maxDistance);
  const opacityY = Math.max(0, 1 - offsetY / maxDistance);

  darkenEffect.style.background = `radial-gradient(circle, rgba(0, 0, 0, ${Math.min(
    opacityX,
    opacityY
  )}) 10%, transparent)`;
});
